...
from utils.sub_modules import (
    SparseMultiHeadAttention
)


class SparseTemporalFusionTransformer(BaseModelWithCovariates):
    def __init__(
        self,
        alpha: float = 1.5,
        hidden_size: int = 16,
        ...
        **kwargs,
        ...
        # )
        self.alpha = alpha
        self.multihead_attn = SparseMultiHeadAttention(
            d_model=self.hparams.hidden_size, n_head=self.hparams.attention_head_size, dropout=self.hparams.dropout, alpha=self.alpha
        )
    ...
    def epoch_end(self, outputs, label="train"):
        ...
        if self.log_interval > 0:
            ...
            self.log_interpretation(outputs, label)
    ...
    def log_interpretation(self, outputs, label="train"):
        """
